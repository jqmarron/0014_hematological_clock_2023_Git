...
## RELATIONSHIP ACC and OTHER PHENOTYPES: 
Goal: Determine eventual association between age acceleration and other phenotypes.
1) Frailty index on those animals with both blood samples and frailty analyses. 
2) Body Weight (Not appropriate analysis because BW is a feature already used in the DNN to calculate acceleration)

***
```{r setup, include=FALSE}
Sys.setlocale(category = "LC_ALL", locale = "Greek")
knitr::opts_chunk$set(echo = TRUE)
patho<-"W:/0014_Hematological_Clock_2023/"
...
```{r}
FI$ID<-paste0("C",FI$idno)#filter varaibles of interest
FI<-FI %>% select("ID",
...
Ages in months in Advia dataset 
```{r}
...
Build and key ID+age in months to merge FI and blood datasets
```{r}
...
Merge datasets by ID+Age in dataset BI
```{r}
...
Include a knot at top of curve to compare regression with piece-wise regression

Strategy: Find breaking point in the acceleration curve and split the regression into two. Acceleration up to the point and the acceleration after the point. Two regressions, four estimates. Find the point by fitting the lm that minimizes the residual std error (scaled Acceleration)

Possible breaking points: Sort the FI and avoid tails.
```{r}
...
31 points in where to break FI; Lets remove tails.
...
Find the model that minimize std error splitin on each break
```{r}
d<-numeric(29)
df$FI<-
for (i in 1:29){
...
Residualize acceleration to solve the high correlation detected in the correlation matrix.
```{r}
