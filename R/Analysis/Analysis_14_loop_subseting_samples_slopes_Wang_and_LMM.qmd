---
title: "Table LMM and Wang slope-acceleration sub-setting by age" 
...
Version of the analysis 9 including filtering the analyses to include different subset of samples by Age only in the analysis involving slope-acceleration.
Goal: Determine validity of the models. Sub-setting starting from samples up to 55 weeks age up to 130 weeks by increments of 5 weeks -samples  (Line 879+)
...
save(AA,file = "/data/martinezromerj2/0016_Hematological_clock_2023/Tables/Output/All_animals_natural_death.Rdata")}
```
...
samples beyond week 81 in the Q1 sub-population. Sub sample Q3 to
maximum age in Q1 equals to 81. This makes the two groups to have same
time-points.
```{r}
cat("Sex: ",params$sex,". Strain: ",params$strain,". Acceleration = ", params$method,". Dataset = ", params$dataset)
```
...
mod11 <- lmerTest::lmer(formula4, subset(AB, age_wk <= 81))
summary(mod11, cor=T)
```
...
Goal: Determine validity of the models using different subset of samples starting from samples up to 55 weeks age up to 130 weeks by increments of 5 weeks 
...
df<- CC %>%
  ...
  filter(n() > 2) %>% filter(final_natural_death==1) %>% 
    filter(age_wk<=x) # remove non natural death
df<- data.table(df)
df<-as.data.frame(df)
...
summary(mo_sp)
    a<-summary(mo_sp)$coefficients[2,1]
    b<-summary(mo_sp)$coefficients[2,5]
temp_df <- data.frame(
    Dataset = x,
    Slope_Beta_LMM=a,
    Slope_pval_LMM=b,
...
```{r}
results_LMM_T1vsT3<-results
write.csv(results_LMM_T1vsT3,file="/data/martinezromerj2/0016_Hematological_clock_2023/Tables/Output/results_LMM_T1vsT3.csv")
```
