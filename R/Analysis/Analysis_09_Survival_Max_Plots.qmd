...
Update survival after final diagnosis by NIA-NIH pathologist (CMS)
...
update_mortality<-function(df){
      ...
      #Update natural death censor and lifespan according to final NIA-CMS report
...
for the same ID across age in extended models.
...
### LMM to test acceleration in long and short survivors

LMM to check whether belonging to long(short)-survival group (Lifespan Tercile
3) encompasses slower(faster) age-acceleration.
...
LMM to test acceleration and lifespan as a categorical variable
```{r}
#Filter only animals living up to 105 weeks to compare same timepoint in both groups long and short survivors
...
## Maximun Lifespan analysis (third aproach)
Find animals with 2 or more blood samples, instance of natural deaht and extract the slope of their acceleration trajectories. Then rank the slopes and determine quantiles. Finally, compare maximum lifespan calculated as described in the paper by Wang et all (2004) between groups with higher acceleration SLOPE versus lower (T1vsT3)
...
```{r}
#Filter animals with at least 2 data points (slope)
df<- CC %>%
  group_by(ID) %>%
...
```{r}
#Extract the slopes of those trajectories per animal and rank in terciles
nn<-3 #Quan-ciles
...
#### Accelerated group according to the slope (T3)
```{r}
...
```
***
#### Decelerated group according to the slope (T1)
```{r}
...
```{r}
#Determine the P90 lifespan and count animals living above that threshold 
...
Rationale to apply: The table we obtain is unconditional (meaning we didn't know the sums for rows or column when designing the experiment). It requires an unconditional test. Fisher's test is fully conditional, it could be opposed in this setting. Boschloo's test is an exact unconditional test for 2x2 tables based on ordering the sample space by Fisher's exact p-values.
...
Compare proportion of animals living longer than P90th weeks with animals living shorter than P90th weeks 
```{r, eval=TRUE}
